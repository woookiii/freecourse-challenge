...
배치프로세스를 하듯이 정해둔 시간 간격마다 카프카에 변경사항에 대한 메세지를 발행하여 
캐싱과 서칭, replica db에 대한 master db와의 동기화를 하는 것보다 
더 이른 시점에(사실상 실시간) 메시지를 발행하고 더 staleless하게 
동기화를 진행할 수 있다고 생각했습니다.
...
오픈서치 grpc 클라이언트를 grpc를 어떻게 사용하는 것인지 이번 프리코스를 통해서 공부하게 되었으니까
다음엔 사용해보기로 했습니다.
...
과거에 자바 스프링을 통해서 @KafkaListener를 통해서 소비를 할때 topic을 기준으로 consumer group을 통해서
...
기억할 수 있었고, 또, 카프카의 컨슈머를 구성할 때, 새로운 고루틴의 무한 루프를 통해서 
계속해서 카프카를 listen하는 구조를 직접 만들어보면서, @KafkaListener, @Controller 
스프링 어노테이션이 내부적으로 이와 같이 동작을 하고 있음을 추측해볼 수 있었습니다.
